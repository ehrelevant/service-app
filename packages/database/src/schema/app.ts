...
export const userRole = app.table('user_roles', {
  userId: uuid().notNull().references(() => user.id, { onUpdate: 'cascade' }),
  roleId: uuid().notNull().references(() => role.id, { onUpdate: 'cascade' }),
