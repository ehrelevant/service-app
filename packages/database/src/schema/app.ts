...
export const provider = app.table(
  'provider',
  {
    userId: uuid('user_id').primaryKey().notNull().references(() => user.id, { onUpdate: 'cascade', onDelete: 'cascade' }),
...
export const serviceType = app.table(
  "service_type",
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
    name: text('name').notNull(),
...
export const service = app.table(
  "service",
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
    serviceTypeId: uuid('service_type_id').notNull().references(() => serviceType.id, { onUpdate: 'cascade', onDelete: 'restrict' }),
    providerUserId: uuid('provider_id').notNull().references(() => provider.userId, { onUpdate: 'cascade', onDelete: 'cascade' }),
...
export const portfolio = app.table(
  "portfolio",
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
...
export const portfolioImage = app.table(
  "portfolio_image",
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
    portfolioId: uuid('portfolio_id').notNull().references(() => portfolio.id, { onUpdate: 'cascade', onDelete: 'cascade' }),
...
export const reviewImage = app.table(
  "review_image",
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
    reviewId: uuid('review_id').notNull().references(() => review.id, { onUpdate: 'cascade', onDelete: 'cascade' }),
    image: text('image').notNull(),
  }
);
...
export const request = app.table(
  "request",
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
    serviceTypeId: uuid('service_type_id').notNull().references(() => serviceType.id, { onUpdate: 'cascade', onDelete: 'restrict' }),
    seekerUserId: uuid('seeker_id').notNull().references(() => seeker.userId, { onUpdate: 'cascade', onDelete: 'cascade' }),
    addressId: uuid('address_id').notNull().references(() => address.id, { onUpdate: 'cascade', onDelete: 'cascade' }),
    description: text('description'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true })
      .notNull()
...
export const requestImage = app.table(
  "request_image",
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
    requestId: uuid('request_id').notNull().references(() => request.id, { onUpdate: 'cascade', onDelete: 'cascade' }),
    image: text('image').notNull(),
  }
);
...
export const booking = app.table(
  "booking",
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
    providerUserId: uuid('provider_id').notNull().references(() => provider.userId, { onUpdate: 'cascade', onDelete: 'cascade' }),
...
export const address = app.table(
  "address",
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
...
export const message = app.table(
  ...
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
    bookingId: uuid('booking_id').notNull().references(() => booking.id, { onUpdate: 'cascade', onDelete: 'cascade' }),
    userId: uuid('user_id').notNull().references(() => user.id, { onUpdate: 'cascade', onDelete: 'cascade' }),
...
export const messageImage = app.table(
  "message_image",
  {
    id: uuid('id').primaryKey().notNull().defaultRandom(),
    messageId: uuid('message_id').notNull().references(() => message.id, { onUpdate: 'cascade', onDelete: 'cascade' }),
    image: text('image').notNull(),
  }
);
